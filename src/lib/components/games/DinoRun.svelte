<script lang="ts">
  ...
  function playBgmNote() {
    try {
      ...
      osc.type = 'sawtooth';
      osc.frequency.setValueAtTime(bgmNotes[currentNoteIndex], audioCtx.currentTime);
  ...
  function drawDino() {
    ...
    if (dino.activePowerUp) {
       ctx.fillStyle = dino.activePowerUp === 'SPEED' ? '#facc15' : (dino.activePowerUp === 'GHOST' ? '#c084fc' : '#4ade80');
  ...
  function update(dt: number) {
    ...
    let speedMultiplier = dino.activePowerUp === 'SPEED' ? 2 : (dino.activePowerUp === 'FLY' ? 1.5 : 1);
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
      ...
      if (dino.activePowerUp !== 'GHOST' && dino.activePowerUp !== 'FLY') {
  ...
  function draw() {
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`LVL: ${level} | SCORE: ${Math.floor(score).toString().padStart(5, '0')}`, canvas.width - 20, 30);
    ...
    if (!gameStarted) {
      ...
      ctx.fillText('READY TO INITIATE', canvas.width / 2, canvas.height / 2 - 10);
      
      ctx.fillStyle = '#a1a1aa';
      ctx.font = '14px monospace';
      ctx.fillText('Press SPACE or Tap to start', canvas.width / 2, canvas.height / 2 + 25);
      ctx.textAlign = 'left'; // reset
    } else if (isGameOver) {
      ctx.fillStyle = '#f43f5e'; // rose-500
      ctx.font = 'bold 28px monospace';
      ctx.textAlign = 'center';
      ctx.fillText('SYSTEM COMPROMISED', canvas.width / 2, canvas.height / 2 - 10);
      
      ctx.fillStyle = '#a1a1aa';
      ctx.font = '14px monospace';
      ctx.fillText('Press SPACE or Tap to reboot', canvas.width / 2, canvas.height / 2 + 25);
      ctx.textAlign = 'left'; // reset
    }
  }
...
<div class="w-full flex flex-col items-center justify-center pt-24 px-4 gap-6 min-h-[80vh]">
  ...
  <div class="text-zinc-500 font-mono text-sm max-w-3xl w-full text-center mt-2 flex flex-col gap-1">
    <div>
